...
- **Rate Calculation**: Place orders at **FRR (Flash Return Rate) * 0.98**.
  - *Optimization*: If the current market Order Book rate is excellent (Book Rate * 0.98 > FRR * 0.98), it will automatically increase the rate to capture higher yield.
...
> Docker mounts the `logs` directory at startup. If this directory doesn't exist locally, Docker usually creates it automatically, but permissions might belong to root. It's recommended to create it manually first to ensure correct permissions, or let Docker handle it.
