...
- **利率計算**：以 **FRR (Flash Return Rate) * 0.98** 掛單。
  - *優化*：若當下市場掛單簿 (Order Book) 的利率極佳 (Book Rate * 0.98 > FRR * 0.98)，則會自動提升利率以獲取更高收益。
- **超時機制**：掛單後等待 **1 分鐘**。
...
> Docker 啟動時我們會掛載 `logs` 目錄，若本機該目錄不存在，Docker 通常會自動建立，但權限可能會歸屬於 root。建議可先手動建立以確保權限正確，或直接讓 Docker 處理。
